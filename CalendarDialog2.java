...
public class CalendarDialog2 extends JDialog implements ChangeListener {
	...
	public CalendarDialog2(ManagerRsvpViewFrame managerRsvpViewFrame, CalendarModel model){
		super(managerRsvpViewFrame, "Calendar",true);
		JPanel calendarPanel = new JPanel();
		calendar = model;
		calendarLabel = new JLabel();
		calendarPanel.setLayout(new BorderLayout());
		
		//set up navigation and label
		//label
		...
		for(int i = 0; i < row.length; i++){
			...
			daybuttons[i].addActionListener(event->{
				...
				if(!chosenButton.getText().equals("")){
					...
					managerRsvpViewFrame.update(dateChosen);	// had a boolean firstField before
				}
			});
